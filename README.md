### Hey, I'm Hunter Veltri
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Hunter%20Veltri-0A66C2?style=flat&logo=linkedin)](https://linkedin.com/in/hunter-veltri-0b6926370) [![Website](https://img.shields.io/badge/Website-hunter.cv-000?style=flat&logo=vercel)](https://hunter.cv)

AI engineer shipping production multi-agent systems across many models (Anthropic, OpenAI, xAI, MiniMax, local) with routing, retrieval, and evals I wrote myself. Built and run the ATFG AI Platform solo to ~100 paying financial producers at 92% month-over-month retention.

#### Building
<!-- BUILDING:START -->
- **[hunter.cv](https://hunter.cv)** - Interactive AI CV: agentic RAG, an eval harness that grades it on 38 questions, and 6-layer security, all on free tiers under $5/mo.
- **[vibebloat](https://github.com/veltri-23/vibebloat)** - Reads your agent history and compiles every mistake so the same one is never made twice. Zero token cost.
<!-- BUILDING:END -->

#### Contributing to
<!-- CONTRIB:START -->
- **[Printing Press Library](https://github.com/mvanhorn/printing-press-library)** - Authored the Paperclip CLI documentation published in the library: install and auth across Node.js, Hermes, OpenClaw, and Claude Desktop, 40+ command groups, and agent-native usage. Credited on the page as Created by @veltri-23.
<!-- CONTRIB:END -->
